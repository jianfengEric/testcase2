    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            if(StringUtils.isNotBlank(accessToken)){
                TokenType tokenType = JWTTokenUtil.getTokenType(accessToken);
                if(TokenType.USER_TOKEN.equals(tokenType)){
                    ...
                    if(null!=userDetails){
                        ...
                        writeResponse((HttpServletResponse)response, 403, PropertiesUtil.getServiceName()+" Not Account found with user name");
                    }
                }
            }
