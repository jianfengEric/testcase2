...
public class ApprovalServiceImpl implements ApprovalService{
    ...
    @Override
    public PageDatas<ApprovalResponseDto> getApprovalList(ApprovalRequestDto pageRequestDto) {
    ...
    private Specifications<ApprovalResult> getWhere(ApprovalRequestDto pageRequestDto){
        ...
        if(StringUtils.isNotBlank(pageRequestDto.getApprovalDate())){
            where = where.and(ApprovalResultSpecifications.approvalDateLike(pageRequestDto.getApprovalDate()));
        }
        if(StringUtils.isNotBlank(pageRequestDto.getRequestDate())){
            where = where.and(ApprovalResultSpecifications.requestDateLike(pageRequestDto.getRequestDate()));
        }
    ...
    private DeployDetailDto getParticipantAndProgramAndGateway(String geaRefId, FullCompanyInfoDto fullCompanyInfoDto, GatewaySettingDto gatewaySettingDto, DeployDetailDto deployDetailDto) {
        ...
        gatewaySettingDto.getOldApiGatewaySettingDto().stream().findFirst().ifPresent(item -> {
            ...
            ewpApiGatewayConfigDeployDetailDto.setServerLogEnableEncrypt(item.getServerLogEncryptionKey());
        ...
        serviceSettingRequestDto.getOldServiceSettingDtoList().forEach(item -> {
            ...
            item.getFromCurrencyDto().forEach(i -> {
                i.getToCurrencyDto().forEach(o -> {
                    ...
                    currencyDeployDetailDto.setAdminFee(o.getAdminFee().toString());
